...
def admin_required(view_func):
    """
    Decorator for views that checks that the user is logged in and is a superuser or admin role,
...
def designer_required(view_func):
    """
    Decorator for views that checks that the user is logged in and is a designer.
    """
    ...
    def _wrapped_view(request, *args, **kwargs):
        if not request.user.is_authenticated:
            return redirect(reverse("user_auth:login"))
        profile = getattr(request.user, "profile", None)
        is_designer = request.user.is_superuser or (profile and profile.is_designer)
        if not is_designer:
            raise PermissionDenied
        return view_func(request, *args, **kwargs)
...
def customer_required(view_func):
    ...
    def _wrapped_view(request, *args, **kwargs):
        if not request.user.is_authenticated:
            return redirect(reverse("user_auth:login"))
        profile = getattr(request.user, "profile", None)
        is_customer = request.user.is_superuser or (profile and profile.is_customer)
        if not is_customer:
            raise PermissionDenied
        return view_func(request, *args, **kwargs)
